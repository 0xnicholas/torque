...
fn test_delegation_status_display() {
    ...
    assert_eq!(DelegationStatus::TimeoutPartial.to_string(), "TIMEOUT_PARTIAL");
}

#[test]
fn test_delegation_status_try_from_str() {
    assert_eq!(DelegationStatus::try_from("PENDING").unwrap(), DelegationStatus::Pending);
    assert_eq!(DelegationStatus::try_from("COMPLETED").unwrap(), DelegationStatus::Completed);
    assert_eq!(DelegationStatus::try_from("FAILED").unwrap(), DelegationStatus::Failed);
