...
async fn memory_candidate_api_tests_list_memory_candidates_is_project_scoped() {
    ...
    let scoped = repo
        .list_candidates(&scope_one, 10, 0)
        .await
...
async fn memory_candidate_api_tests_create_memory_entry_persists_project_scope_and_status() {
    ...
    let scoped = repo
        .list_entries(&project_scope, 10, 0)
        .await
...
async fn memory_candidate_api_tests_reject_cross_project_candidate_link_on_entry_create() {
    ...
    assert!(
        result.is_err(),
        "cross-project candidate linkage should be rejected"
    );
}
...
async fn memory_candidate_api_tests_entry_status_transition_clears_invalidated_timestamp() {
    ...
    let replaced = repo
        .update_entry_status(&project_scope, saved_entry.id, MemoryEntryStatus::Replaced)
        .await
    ...
    let active = repo
        .update_entry_status(&project_scope, saved_entry.id, MemoryEntryStatus::Active)
        .await
...
async fn memory_candidate_api_tests_replace_entry_marks_old_replaced_and_creates_new_entry() {
    ...
    let old_after = memory_repo
        .get_entry_by_id(&session.project_scope, old_entry.id)
        .await
        .expect("query should succeed")
        .expect("old entry should exist");
    assert!(matches!(old_after.status, MemoryEntryStatus::Replaced));

    let all_entries = memory_repo
        .list_entries(&session.project_scope, 100, 0)
        .await
        .expect("entries should list");
    assert!(
        all_entries
            .iter()
            .any(|entry_row| entry_row.id == new_entry_id),
...
async fn memory_candidate_api_tests_invalidate_entry_marks_entry_invalidated() {
    ...
    assert_eq!(
        invalidate_json["status"],
        Value::String("Invalidated".to_string())
    );

    let stored = memory_repo
        .get_entry_by_id(&session.project_scope, saved.id)
        .await
        .expect("query should succeed")
        .expect("entry should exist");
...
async fn memory_candidate_api_tests_search_memory_endpoint_returns_ranked_active_entries() {
    ...
    assert_eq!(
        entries.len(),
        2,
        "should only include active entries in scope"
    );
    assert_eq!(
        entries[0]["id"],
