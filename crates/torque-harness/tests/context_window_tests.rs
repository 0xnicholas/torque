use torque_harness::agent::context::{ContextManager, DEFAULT_WINDOW_SIZE};
use torque_harness::models::Message;
use uuid::Uuid;
...
fn context_manager_only_keeps_recent_window() {
    ...
    assert_eq!(
        context.messages.first().expect("first message").content,
        "message-15"
    );
    assert_eq!(
        context.messages.last().expect("last message").content,
        "message-24"
    );
}
