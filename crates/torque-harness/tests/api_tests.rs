...
async fn test_create_session() {
    ...
    assert!(matches!(
        session.status,
        torque_harness::models::SessionStatus::Idle
    ));
}
...
async fn test_session_status_transitions() {
    ...
    repo.update_status(
        session.id,
        torque_harness::models::SessionStatus::Running,
        None,
    )
    .await
...
async fn test_api_key_isolation() {
    ...
    let session_1 = repo
        .create(&api_key_1, "scope-1")
        .await
        .expect("Failed to create session 1");

    let _session_2 = repo
        .create(&api_key_2, "scope-2")
        .await
        .expect("Failed to create session 2");

    let sessions_1 = repo.list(10).await.expect("Failed to list sessions");

    let sessions_1: Vec<_> = sessions_1
        .into_iter()
        .filter(|s| s.api_key == api_key_1)
        .collect();
...
async fn test_try_mark_running_is_atomic_gate() {
    ...
    let repo = PostgresSessionRepository::new(db);
    let session = repo
        .create("key-1", "test-scope")
        .await
        .expect("Failed to create session");

    let first = repo
        .try_mark_running(session.id)
        .await
