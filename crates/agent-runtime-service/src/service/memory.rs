...
use crate::models::v1::memory::{
    ...
    MemoryCategory, SemanticSearchResult, SemanticSearchQuery, SessionMemoryEntry,
    SessionMemorySet, MemoryDecisionLog,
};
use crate::repository::{MemoryRepository, MemoryRepositoryV1};
use crate::embedding::{EmbeddingGenerator, memory_to_embedding_text};
use std::sync::Arc;
use uuid::Uuid;
...
impl MemoryService {
    ...
    pub async fn v1_create_entry(
        ...
        self.repo_v1.create_entry_with_embedding(
            agent_instance_id,
            team_instance_id,
            category,
            key,
    ...
    pub async fn v1_semantic_search(
        ...
        let embedding_gen = self.embedding.as_ref()
    ...
    pub async fn v1_update_candidate_status(
        ...
        self.repo_v1.update_candidate_status(id, status, reviewed_by, memory_entry_id).await
    }
    ...
    pub async fn session_memory_set(
        ...
        self.repo_v1.session_memory_set(session_id, &req.key, req.value.clone(), expires_at).await
    }

    pub async fn session_memory_delete(
        &self,
        session_id: Uuid,
        key: &str,
    ) -> anyhow::Result<bool> {
    ...
    pub async fn backfill_embedding(
        ...
        self.repo_v1.update_entry_embedding(entry_id, &embedding, &model).await
    }
}
