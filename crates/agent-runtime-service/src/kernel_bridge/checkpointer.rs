...
impl Checkpointer for PostgresCheckpointer {
    async fn save(
        ...
        .await
        .map_err(|e| checkpointer::CheckpointerError::Database(e))?;
        Ok(CheckpointId(id))
    }
    ...
    async fn load(&self, checkpoint_id: CheckpointId) -> checkpointer::Result<CheckpointState> {
        ...
        let (json,) = row.ok_or_else(|| checkpointer::CheckpointerError::NotFound(
            format!("checkpoint not found: {}", checkpoint_id.0)
        ))?;
        serde_json::from_value(json)
            .map_err(|e| checkpointer::CheckpointerError::Serialization(e.to_string()))
    }

    async fn list_run_checkpoints(&self,
        run_id: Uuid,
    ) -> checkpointer::Result<Vec<CheckpointMeta>> {
        let rows: Vec<(Uuid, Uuid, DateTime<Utc>)> = sqlx::query_as(
            "SELECT id, task_id, created_at FROM checkpoints WHERE instance_id = $1 ORDER BY created_at DESC"
        )
        .bind(run_id)
    ...
    async fn list_node_checkpoints(&self,
        node_id: Uuid,
    ) -> checkpointer::Result<Vec<CheckpointMeta>> {
        let rows: Vec<(Uuid, Uuid, DateTime<Utc>)> = sqlx::query_as(
            "SELECT id, instance_id, created_at FROM checkpoints WHERE task_id = $1 ORDER BY created_at DESC"
        )
        .bind(node_id)
        .fetch_all(self.db.pool())
        .await
