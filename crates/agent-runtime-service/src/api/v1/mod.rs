...
pub fn router() -> Router<(Database, Arc<OpenAiClient>, Arc<ServiceContainer>)> {
    Router::new()
        ...
        .route("/v1/artifacts/:id", get(artifacts::get).delete(artifacts::delete))
        ...
        .route("/v1/capability-profiles", post(capabilities::create_profile).get(capabilities::list_profiles))
        .route("/v1/capability-profiles/:id", get(capabilities::get_profile).delete(capabilities::delete_profile))
        .route("/v1/capability-profiles/:id/resolve", post(capabilities::resolve))
        .route("/v1/capability-registry-bindings", post(capabilities::create_binding).get(capabilities::list_bindings))
        .route("/v1/capability-registry-bindings/:id", get(capabilities::get_binding).delete(capabilities::delete_binding))
        .route("/v1/team-definitions", post(teams::create_definition).get(teams::list_definitions))
        .route("/v1/team-definitions/:id", get(teams::get_definition).delete(teams::delete_definition))
        .route("/v1/team-instances", post(teams::create_instance).get(teams::list_instances))
        .route("/v1/team-instances/:id", get(teams::get_instance).delete(teams::delete_instance))
        .route("/v1/team-instances/:id/tasks", get(teams::list_tasks).post(teams::create_task))
        .route("/v1/team-instances/:id/members", get(teams::list_members))
        .route("/v1/team-instances/:id/publish", post(teams::publish))
        .route("/v1/delegations", post(delegations::create).get(delegations::list))
        ...
        .route("/v1/memory-write-candidates", post(memory::create_candidate).get(memory::list_candidates))
        .route("/v1/memory-write-candidates/:id", get(memory::get_candidate))
        .route("/v1/memory-write-candidates/:id/approve", post(memory::approve_candidate))
