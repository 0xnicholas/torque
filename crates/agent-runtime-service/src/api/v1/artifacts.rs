...
pub async fn create(
    ...
    let artifact = services.artifact.create(&req.kind, req.scope, &req.mime_type, req.content
    ).await.map_err(|e| (
...
pub async fn list(
    ...
    let mut rows = services.artifact.list(limit + 1).await
        .map_err(|e| (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorBody { code: "DB_ERROR".into(), message: e.to_string(), details: None, request_id: None })
        ))?;
...
pub async fn publish(
    ...
    match services.artifact.update_scope(id, ArtifactScope::TeamShared).await {
        Ok(true) => Ok(StatusCode::NO_CONTENT),
        Ok(false) => Err(StatusCode::NOT_FOUND),
        Err(_) => Err(StatusCode::INTERNAL_SERVER_ERROR),
    }
