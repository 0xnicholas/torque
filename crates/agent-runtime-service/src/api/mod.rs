...
pub fn router(db: Database, llm: Arc<OpenAiClient>, services: Arc<ServiceContainer>) -> Router {
    ...
    Router::new()
        ...
        .route("/sessions/:id/memory/candidates/:candidate_id/accept", post(memory::accept_candidate))
        .route("/sessions/:id/memory", get(memory::list_entries))
        .route("/sessions/:id/memory/search", get(memory::search_entries))
        .route("/sessions/:id/memory/:entry_id/replace", post(memory::replace_entry))
        .route("/sessions/:id/memory/:entry_id/invalidate", post(memory::invalidate_entry))
