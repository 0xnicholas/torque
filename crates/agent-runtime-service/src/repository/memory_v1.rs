...
use crate::models::v1::memory::{
    MemoryEntry, MemoryEntryRow, MemoryWriteCandidate, MemoryWriteCandidateStatus, MemoryCategory,
    SemanticSearchResult, SemanticSearchRow, HybridSearchRow, SessionMemoryEntry, MemoryDecisionLog,
};
use uuid::Uuid;
...
pub trait MemoryRepositoryV1: Send + Sync {
    ...
    async fn session_memory_delete(
        &self,
        session_id: Uuid,
        key: &str,
    ) -> anyhow::Result<bool>;
    ...
    // Backfill
    async fn get_entries_without_embedding(
        &self,
        limit: i64,
    ) -> anyhow::Result<Vec<MemoryEntry>>;
...
impl MemoryRepositoryV1 for PostgresMemoryRepositoryV1 {
    ...
    async fn get_entry_by_id(
        ...
        let row = sqlx::query_as::<_, MemoryEntryRow>(
            r#"SELECT * FROM v1_memory_entries WHERE id = $1"#,
        )
        .bind(id)
    ...
    async fn session_memory_delete(
        ...
        let result = sqlx::query(
            r#"DELETE FROM session_memory WHERE session_id = $1 AND key = $2"#,
        )
        .bind(session_id)
        .bind(key)
