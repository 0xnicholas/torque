...
use agent_runtime_service::agent::stream::StreamEvent;
use agent_runtime_service::models::v1::agent_definition::AgentDefinitionCreate;
use agent_runtime_service::models::v1::agent_instance::{AgentInstanceCreate, AgentInstanceStatus};
...
use agent_runtime_service::service::{RunService, ToolService};
use agent_runtime_service::kernel_bridge::PostgresCheckpointer;
...
async fn test_run_lifecycle_creates_task_and_updates_instance_status() {
    ...
    let result = run_service.execute(instance.id, run_request, event_tx).await;
    ...
    let task = tasks.iter().find(|t| t.agent_instance_id == Some(instance.id));
    assert!(task.is_some(), "Task was not created for instance");
    
    let task = task.unwrap();
    assert_eq!(task.goal, "Say hello");
    assert!(matches!(task.status, TaskStatus::Completed), 
        "Task should be Completed, got {:?}", task.status);

    // 7. Verify instance status updated to Ready
    let updated_instance = inst_repo.get(instance.id).await.expect("get instance").unwrap();
    assert!(matches!(updated_instance.status, AgentInstanceStatus::Ready),
        "Instance should be Ready, got {:?}", updated_instance.status);
    assert_eq!(updated_instance.current_task_id, None,
        "Instance should not have current task after completion");

    // Cleanup
    inst_repo.delete(instance.id).await.expect("delete instance");
    def_repo.delete(definition.id).await.expect("delete definition");
}
...
async fn test_run_with_nonexistent_instance_returns_error() {
    ...
    let run_service = RunService::new(
        def_repo, inst_repo, task_repo, event_repo, checkpoint_repo,
        checkpointer, llm, tools,
    );
    ...
    assert!(has_error, "Should receive Error event for nonexistent instance");
}
...
async fn test_run_task_status_transitions() {
    ...
    run_service.execute(instance.id, run_request, event_tx).await.expect("execute run");
    ...
    let updated_instance = inst_repo.get(instance.id).await.expect("get instance").unwrap();
    assert!(
        matches!(updated_instance.status, AgentInstanceStatus::Ready | AgentInstanceStatus::Failed),
        "Instance should be in terminal state, got {:?}", updated_instance.status
    );
