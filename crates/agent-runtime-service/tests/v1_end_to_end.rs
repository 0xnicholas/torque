...
use agent_runtime_service::models::v1::artifact::{ArtifactScope};
...
async fn test_v1_agent_definition_lifecycle() {
    ...
    let created = repo.create(&AgentDefinitionCreate {
    ...
    let deleted = repo.delete(created.id).await.expect("delete agent definition");
    assert!(deleted);
...
async fn test_v1_agent_instance_lifecycle() {
    ...
    let fetched = inst_repo.get(instance.id).await.expect("get agent instance");
    assert!(fetched.is_some());

    inst_repo.delete(instance.id).await.expect("delete agent instance");
    def_repo.delete(definition.id).await.expect("delete agent definition");
}
...
async fn test_v1_artifact_lifecycle() {
    ...
    let published = repo.update_scope(artifact.id, ArtifactScope::TeamShared).await.expect("publish artifact");
    assert!(published);
...
async fn test_v1_list_pagination() {
    ...
    let list = repo.list(3, None, None).await.expect("list agent definitions");
    assert_eq!(list.len(), 3);

    let all = repo.list(10, None, None).await.expect("list all agent definitions");
    assert_eq!(all.len(), 5);
}
