...
- reuse the current `session-agent` path instead of building a second throwaway demo stack
...
The MVP should reuse the current `crates/session-agent` direction and tighten it into a clean demo path.
